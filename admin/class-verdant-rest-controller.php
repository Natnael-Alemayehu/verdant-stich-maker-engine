...
class VerdantStitch_REST_Controller {
    ...
    public function __construct(
        ...
        private VerdantStitch_WooCommerce_Brodge    $wc_bridge,
    ...
    public function register_routes(): void {
        ...
        register_rest_route($ns, '/progress/(?P<id>[\d]+)', [
            [
                ...
                'permission_callback'   => array_merge(
                    $this->user_id_arg(), 
        ...
        register_rest_route( $ns, '/levels', [
            [
                ...
                'permission_callbacl'   => '__return_true', // public read
            ],
        ] );
    }
    ...
    public function add_image( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        $user_id = $this->auth->response_user_id($request);
        if (is_wp_error($user_id)) {
            return $user_id;
        }
     ...
     */
    public function recalculate_mastery( WP_REST_Response $request ): WP_REST_Response|WP_Error {
        $user_id = $this->auth->resolve_user_id($request);
        if( is_wp_error($user_id) ) {
            return $user_id;
        }
    ...
    private function user_id_arg(): array {
        return [
            'user_id' => [
                ...
                'required'              => false,
                'description'           => __('Target user ID. Defaults to authenticated user. Admin only ovveride.', 'verdant-stitch'),
            ],
        ];
    }
