...
class VerdantStitch_Auth {
     ...
     *
     * Works with Application Passwords (Basic Auth) and cookie sessions.
     *
     ...
     *  – Regular users can only access their own data; any ?user_id param
    ...
    public function resolve_user_id( WP_REST_Request $request ): int|WP_Error {
        ...
        if ( $param_user_id && $param_user_id !== $current_user_id ) {
            ...
            if ( ! get_userdata( $param_user_id ) ) {
                return new WP_Error(
                    'user_not_found',
                    __( 'The specified user does not exist.', 'verdant-stitch' ),
                    [ 'status' => 404 ]
                );
            }
