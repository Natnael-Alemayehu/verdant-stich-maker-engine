 ...
 *
 * Handles:
 *   – Retrieving / creating kits for a user.
...
class VerdantStitch_Maker_Profile {
    ...
    /**
     * Build a full profile summary for the REST GET endpoint.
     *
     * @param int $user_id
     * @return array
     */
    public function get_profile_summary( int $user_id ): array {
        ...
        return [
            ...
            'mastery'      => [
                ...
                'last_calculated' => $mastery ? $mastery->last_calculated : null,
            ],
    ...
    /**
     * Get or create a kit row for a user.
     *
     * @param int    $user_id
     ...
     * @return object|WP_Error  The kit row object or a WP_Error.
     */
    public function get_or_create_kit( int $user_id, string $kit_id, string $kit_name, int $difficulty = 1, int $total_steps = 10 ): object|WP_Error {
        ...
        return $this->db->get_kit( $row_id, $user_id );
    }

    /**
     * Update step progress for a kit.
     *
     * Validates ownership, step bounds, and status transitions.
     *
    ...
    public function update_progress( int $kit_row_id, int $user_id, int $completed_steps, string $note = '' ): object|WP_Error {
        ...
        $this->db->log_progress( $kit_row_id, $user_id, $completed_steps, $note );
    ...
    public function add_milestone_image( int $kit_row_id, int $user_id, string $image_url, int $step_number = 0, string $caption = '' ): int|WP_Error {
        ...
        if ( ! filter_var( $image_url, FILTER_VALIDATE_URL ) ) {
            return new WP_Error( 'invalid_url', __( 'image_url must be a valid URL.', 'verdant-stitch' ), [ 'status' => 400 ] );
        }

        $image_id = $this->db->insert_image( $kit_row_id, $user_id, $image_url, $step_number, $caption );
        if ( ! $image_id ) {
    ...
    /**
     * Get progress history + images for a kit.
     *
     * @param int $kit_row_id
     * @param int $user_id
    ...
    public function format_kit( object $kit ): array {
        return [
            ...
            'status'           =>          $kit->status,
            'total_steps'      => (int)    $kit->total_steps,
            'completed_steps'  => (int)    $kit->completed_steps,
            'progress_pct'     => $kit->total_steps > 0
                                  ? round( ( $kit->completed_steps / $kit->total_steps ) * 100, 1 )
                                  : 0,
    ...
    public function get_level_label( int $level ): string {
        ...
        return $thresholds[ $level ]['label'] ?? 'Seedling';
    }
}
